...
class SaveMeterService
{
    //get user saved meters
    public function getSavedMeters(){
       return SavedMeter::where('user_id', auth()->user()->id)->orderBy('created_at', 'desc')->get();
    }


    //find meter by if
    public function showMeter(SavedMeter $meter){
        return $meter;
    }

    public function newMeter($meter_number, $disco){
        
        $meter = SavedMeter::firstOrCreate([
